...
class DatabaseTools:
    ...
    def find_nearby_providers(
        ...
        try:
            ...
            logger.error(f"Error finding nearby providers: {e}")
            raise
    ...
    def get_provider_details(self, provider_id: int) -> Optional[ProviderMatch]:
        ...
        try:
            ...
            location_wkt = self.db.scalar(func.ST_AsText(provider.location))
            coords = self._parse_point_wkt(location_wkt) if location_wkt else None
    ...
    def check_slot_availability(self, slot_id: int) -> bool:
        ...
        try:
            slot = self.db.query(TimeSlot).filter(TimeSlot.id == slot_id).first()
    ...
    def find_service_category(self, search_term: str) -> Optional[ServiceCategory]:
        ...
        try:
            ...
            logger.error(f"Error finding service category: {e}")
            return None
